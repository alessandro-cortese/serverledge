...
func main() {
	...
	client, err := profiling.NewKeplerClient(metricsURL, timeout)
	...
	defer cancel()
	snapshot, err := client.ReadContainerSnapshot(ctx, containerID)
	if err != nil {
		fail(err.Error())
	}
	...
	if err := encoder.Encode(output); err != nil {
		fail(fmt.Sprintf("encode probe result: %v", err))
	}
}

func fail(message string) {
	fmt.Fprintf(os.Stderr, "kepler-probe: %s\n", message)
	os.Exit(1)
}
