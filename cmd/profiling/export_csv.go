...
func runExportCSV(
	...
	inputPath :=
		flags.String(
			"input",
			profiling.
				DefaultFunctionProfileExportPath,
			"input FunctionProfile JSONL dataset",
		)
	...
	meanOutputPath :=
		flags.String(
			"mean-output",
			"",
			"mean CSV output path; default is next to the input dataset",
		)
	...
	medianOutputPath :=
		flags.String(
			"median-output",
			"",
			"median CSV output path; default is next to the input dataset",
		)
	...
	flags.Usage =
		func() {
			fmt.Fprintf(
				flags.Output(),
				"Usage: serverledge-profiling export-csv [options]\n\n",
			)
