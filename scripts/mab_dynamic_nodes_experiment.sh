...
# Dynamic MAB experiment for Serverledge.
#
# Scenario:
#   1. start etcd
#   2. start only first-architecture nodes, e.g. x86
#   3. start LB in MAB mode
#   4. create a function WITHOUT tag_pattern, so the MAB can choose the ring
...
#
# Usage examples:
#   bash scripts/mab_dynamic_nodes_experiment.sh
#   PRE_REQUESTS=10 POST_REQUESTS=60 SECOND_NODE_CONF=node1-conf.yaml bash scripts/mab_dynamic_nodes_experiment.sh
#
# Notes:
#   - This script demonstrates dynamic discovery and MAB arm activation.
#   - To show that the MAB "learns the better arm", the two node groups must have
#     measurably different response times or a reward/cost metric must be introduced.
#     On a single local machine with simulated tags, the script still proves that
...
FUNCTION_NAME="${FUNCTION_NAME:-mab_dynamic_$(date +%s)}"
CREATE_PORT="${CREATE_PORT:-1324}"
LB_PORT="${LB_PORT:-8080}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
LOG_DIR="${LOG_DIR:-logs/mab_dynamic_${RUN_ID}}"
mkdir -p "$LOG_DIR"

LB_CONF="$LOG_DIR/lb-mab-conf.yaml"
...
echo "[setup] Starting LB in MAB mode with config $LB_CONF"
...
echo
echo "[summary] Relevant LB log lines:"
grep -E "\[LB\]|BanditManager|Using \(forced\)|Score for|Selected target|Adding " "$LOG_DIR/lb.log" | tee "$LOG_DIR/lb_relevant.log" || true
