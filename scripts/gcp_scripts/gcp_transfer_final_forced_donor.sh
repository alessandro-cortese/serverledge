...
require_same_cluster = selection_policy.get(
    "require_same_cluster"
)

forced_donor_ablation = (
    __import__("os")
    .environ.get(
        "FORCED_DONOR_ABLATION",
        "",
...
if require_same_cluster is not True:

    if not (
        forced_donor_ablation
        and require_same_cluster is False
    ):
        raise SystemExit(
            "selection non richiede same cluster "
            "fuori dalla forced-donor ablation"
        )
