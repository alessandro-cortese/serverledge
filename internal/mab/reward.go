...
type RewardCalculator func(feedback ExecutionFeedback) (
	RewardResult,
	error,
)
...
func ConfiguredRewardMode() (RewardMode, error) {

	rawMode := strings.ToLower(strings.TrimSpace(config.GetString(config.MAB_REWARD_MODE, string(RewardModeLatency))))
	if rawMode == "" {
		rawMode = string(RewardModeLatency)
	}

	mode := RewardMode(rawMode)

	if _, ok := rewardCalculators[mode]; !ok {
		return "", fmt.Errorf("unsupported MAB reward mode %q", rawMode)
	}
...
func ValidateRewardConfiguration() error {
	...
	mode, err := ConfiguredRewardMode()
	if err != nil {
		return err
	}

	if mode == RewardModeKeplerEnergy {
		_, err := ConfiguredKeplerRewardZone()
		if err != nil {
			return err
		}
	}
...
func CalculateExecutionReward(feedback ExecutionFeedback) (RewardResult, error) {
	...
	calculator := rewardCalculators[mode]
	return calculator(feedback)
}

// calculateLatencyReward implements the current baseline objective.
func calculateLatencyReward(feedback ExecutionFeedback) (RewardResult, error) {

	if !isFiniteNumber(feedback.DurationMs) {
		return RewardResult{}, fmt.Errorf("duration must be finite, got %v", feedback.DurationMs)
	}

	if feedback.DurationMs <= 0 {
		return RewardResult{}, fmt.Errorf("duration must be positive, got %.6f ms", feedback.DurationMs)
	}

	value := -math.Log(feedback.DurationMs)
