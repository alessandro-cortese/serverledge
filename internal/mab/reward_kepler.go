...
func ConfiguredKeplerRewardZone() (
	string,
	error,
) {
	zone :=
	...
	if zone == "" {
		return "",
			fmt.Errorf(
				"Kepler reward zone is not configured: set %s",
				config.MAB_REWARD_KEPLER_ZONE,
			)
	}
...
func SelectKeplerRewardInput(
	...
	if feedback.KeplerEnergy == nil {
		return KeplerRewardInput{},
			fmt.Errorf(
				"Kepler energy feedback is missing",
			)
	}
	...
	if !feedback.KeplerEnergy.Available {
		reason :=
			strings.TrimSpace(
				feedback.KeplerEnergy.InvalidReason,
			)

		if reason == "" {
			reason =
				"unspecified Kepler collection failure"
		}

		return KeplerRewardInput{},
			fmt.Errorf(
				"Kepler energy feedback is unavailable: %s",
				reason,
			)
	}
	...
	if len(
		...
		return KeplerRewardInput{},
			fmt.Errorf(
				"Kepler energy feedback contains no CPU energy zones",
			)
	}

	joules,
		ok :=
		feedback.KeplerEnergy.CPUJoulesByZone[zone]
	...
	if !ok {
		...
		return KeplerRewardInput{},
			fmt.Errorf(
				"configured Kepler reward zone %q is unavailable; available zones: %v",
				zone,
				availableZones,
			)
	}
	...
	if math.IsNaN(
		...
		return KeplerRewardInput{},
			fmt.Errorf(
				"Kepler energy for zone %q must be finite, got %v",
				zone,
				joules,
			)
	}
	...
	if joules < 0 {
		return KeplerRewardInput{},
			fmt.Errorf(
				"Kepler energy for zone %q cannot be negative, got %.9f J",
				zone,
				joules,
			)
	}
...
func calculateKeplerEnergyReward(
	feedback ExecutionFeedback,
) (
	RewardResult,
	error,
	...
	if input.Joules <= 0 {
		return RewardResult{},
			fmt.Errorf(
				"Kepler energy for reward zone %q must be positive for logarithmic reward, got %.9f J",
				input.Zone,
				input.Joules,
			)
	}
	...
	if math.IsNaN(
		...
		return RewardResult{},
			fmt.Errorf(
				"Kepler logarithmic reward for zone %q is not finite: energy=%.9f J reward=%v",
				input.Zone,
				input.Joules,
				reward,
			)
