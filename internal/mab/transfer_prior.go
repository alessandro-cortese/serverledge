...
func BuildWeakMABPrior(
	...
	if err :=
		validateTransferableKnowledgeForPrior(
			source,
		); err != nil {
	...
	for arm, sourceArm := range source.Arms {
		...
		priorArm :=
			WeakMABArmPrior{
				SourceRealObservationCount: sourceArm.
					RealObservationCount,

				SourceExcludedSyntheticObservationCount: sourceArm.
					ExcludedSyntheticObservationCount,
			}
		...
		case LinUCB:
			...
			priorArm.LinUCB =
				&WeakLinUCBArmPrior{
					...
					AContribution: scaleNestedMatrix(
						sourceLin.
							AContribution,
						scale,
					),

					BContribution: scaleVector(
						sourceLin.
							BContribution,
						scale,
					),
	...
	return WeakMABPrior{
			...
			SourceExcludedSyntheticObservationCount: source.
				ExcludedSyntheticObservationCount,
...
func validateWeakMABPriorConfig(
	config WeakMABPriorConfig,
) error {
	weight :=
		config.
	...
	if !isFiniteNumber(
		...
		return fmt.Errorf(
			"equivalent observation weight must be finite",
		)
	}
	...
	if weight <= 0.0 ||
		...
		return fmt.Errorf(
			"equivalent observation weight must be in (0, 1]",
		)
	}
	...
	if config.
		...
		return fmt.Errorf(
			"minimum real observations per arm must be at least 1",
		)
	}
...
func validateTransferableKnowledgeForPrior(
	source TransferableMABKnowledge,
) error {
	if source.SchemaVersion !=
		TransferableMABKnowledgeSchemaVersion {

		return fmt.Errorf(
			"unsupported transferable MAB knowledge schema version %d",
	...
	if source.FunctionName == "" {
		return fmt.Errorf(
			"donor function name cannot be empty",
		)
	}
	...
	for arm, sourceArm := range source.Arms {
		...
		if sourceArm.
			...
			return fmt.Errorf(
				"arm %q has negative real observation count",
				arm,
			)
		}
		...
		if sourceArm.
			...
			return fmt.Errorf(
				"arm %q has negative synthetic observation count",
				arm,
			)
		}
		...
		case UCB1:
			if sourceArm.UCB1 == nil ||
				sourceArm.LinUCB != nil {

				return fmt.Errorf(
					"arm %q has invalid UCB1 transferable payload",
					arm,
				)
			}

			if err :=
				validateTransferableUCB1Arm(
					arm,
					sourceArm.
						RealObservationCount,
					sourceArm.UCB1,
		...
		case LinUCB:
			if sourceArm.LinUCB == nil ||
				sourceArm.UCB1 != nil {

				return fmt.Errorf(
					"arm %q has invalid LinUCB transferable payload",
					arm,
				)
			}

			if err :=
				validateTransferableLinUCBArm(
					arm,
					sourceArm.LinUCB,
				); err != nil {
	...
	if totalReal !=
		...
		return fmt.Errorf(
			"transferable real observation summary mismatch: arms=%d summary=%d",
			totalReal,
			source.RealObservationCount,
		)
	}
	...
	if totalSynthetic !=
		...
		return fmt.Errorf(
			"transferable synthetic observation summary mismatch: arms=%d summary=%d",
			totalSynthetic,
			source.
				ExcludedSyntheticObservationCount,
		)
	...
	if source.HasRealKnowledge !=
		(totalReal > 0) {

		return fmt.Errorf(
			"has_real_knowledge does not match real observation count",
		)
	}
...
func validateTransferableUCB1Arm(
	arm string,
	realCount int64,
	source *TransferableUCB1ArmKnowledge,
) error {
	if !isFiniteNumber(
		source.RealSumRewards,
	) ||
		!isFiniteNumber(
			source.RealAvgReward,
		) {

		return fmt.Errorf(
			"arm %q has non-finite UCB1 transferable reward statistics",
			arm,
		)
	}

	if realCount == 0 {
		if source.RealSumRewards != 0.0 ||
			source.RealAvgReward != 0.0 {

			return fmt.Errorf(
				"arm %q has UCB1 reward statistics without real observations",
				arm,
			)
		}
	...
	if !weakPriorAlmostEqual(
		expectedAverage,
		source.RealAvgReward,
	) {
		return fmt.Errorf(
			"arm %q has inconsistent UCB1 average reward",
			arm,
		)
	}
...
func validateTransferableLinUCBArm(
	arm string,
	source *TransferableLinUCBArmKnowledge,
) error {
	if source.Dim < 1 {
		return fmt.Errorf(
			"arm %q has invalid LinUCB dimension",
			arm,
		)
	}

	if len(
		source.AContribution,
	) != source.Dim {

		return fmt.Errorf(
			"arm %q has invalid LinUCB A row count",
			arm,
		)
	}
	...
	for i, row := range source.AContribution {
		...
		if len(
			...
			return fmt.Errorf(
				"arm %q has invalid LinUCB A column count at row %d",
				arm,
				i,
			)
		}
		...
		for _, value := range row {
			...
			if !isFiniteNumber(
				...
				return fmt.Errorf(
					"arm %q has non-finite LinUCB A contribution",
					arm,
				)
			}
		}
	...
	if len(
		source.BContribution,
	) != source.Dim {

		return fmt.Errorf(
			"arm %q has invalid LinUCB b dimension",
			arm,
		)
	}
	...
	for _, value := range source.BContribution {
		...
		if !isFiniteNumber(
			...
			return fmt.Errorf(
				"arm %q has non-finite LinUCB b contribution",
				arm,
			)
		}
	}
