...
func Execute(cont *container.Container, r *scheduledRequest, isWarm bool) error {
	...
	if r.Fun.Runtime == container.CUSTOM_RUNTIME {
		req = executor.InvocationRequest{Params: r.Params, ReturnOutput: r.ReturnOutput}
	} else {
	...
	profiling.LogKeplerInvocationEnergyProfile(keplerEnergyProfile)

	r.IsWarmStart = isWarm

	r.Duration = executionWallTime.Seconds() - invocationWait.Seconds()
	...
	executionSucceeded := executionErr == nil && response != nil && response.Success
	...
	sampleTiming := profiling.InvocationTiming{}

	if isWarm {
		sampleTiming.DurationMs = r.Duration * 1000.0
		sampleTiming.ResponseTimeMs = r.ResponseTime * 1000.0
		sampleTiming.QueueingTimeMs = r.QueueingTime * 1000.0
		sampleTiming.OffloadLatencyMs = r.OffloadLatency * 1000.0
		sampleTiming.InvocationWaitMs = float64(invocationWait) / float64(time.Millisecond)
		sampleTiming.ExecutionWallTimeMs = float64(executionWallTime) / float64(time.Millisecond)
	} else {
		sampleTiming.InitTimeMs = r.InitTime * 1000.0
	}

	sample := profiling.BuildInvocationSample(
	...
	defer func() {
		if err := profiling.ExportInvocationSample(sample); err != nil {
			log.Printf("[PROFILING] event=sample_export_failed request_id=%q function=%s error=%q", r.Id(), r.Fun.Name, err.Error())
		}
	}()
	...
	if executionErr != nil {
		...
		return fmt.Errorf("[%s] Execution failed on container %v: %v ", r, cont.ID, executionErr)
	}
