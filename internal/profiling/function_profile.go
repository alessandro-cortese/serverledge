...
func AggregateProfilingFeatureVectors(
	...
	if len(vectors) > MaxFunctionProfileSamples {
		return FunctionProfile{},
			fmt.Errorf(
				"too many profiling samples: got %d, maximum is %d",
				len(vectors),
	...
	if err := validateVectorIdentity(first); err != nil {
		return FunctionProfile{},
			fmt.Errorf(
				"invalid first profiling vector: %w",
				err,
			)
	}
	...
	for index, vector := range vectors {
		if err := validateVectorIdentity(vector); err != nil {
			return FunctionProfile{},
				fmt.Errorf(
					"invalid profiling vector at index %d: %w",
					index,
					err,
				)
		}
		...
		if len(values) != featureCount {
			return FunctionProfile{},
				fmt.Errorf(
					"profiling vector %q contains %d features, expected %d",
					vector.RequestID,
					len(values),
					featureCount,
				)
		...
		for featureIndex, value := range values {
			if !finiteNonNegative(value) {
				return FunctionProfile{},
					fmt.Errorf(
						"profiling vector %q contains invalid feature %s=%v",
						vector.RequestID,
						names[featureIndex],
						value,
					)
			...
			featureColumns[featureIndex] = append(
				featureColumns[featureIndex],
				value,
			)
		}
	}
	...
	for featureIndex, values := range featureColumns {
		meanValues[featureIndex] = meanFloat64(
			values,
		)

		medianValues[featureIndex] = medianFloat64(
			values,
		)
	}

	meanFeatures, err := randomForestFeaturesFromValues(
		meanValues,
	)
	...
	if err != nil {
		return FunctionProfile{},
			fmt.Errorf(
				"failed to build mean feature vector: %w",
				err,
			)
	}

	medianFeatures, err := randomForestFeaturesFromValues(
		medianValues,
	)
	...
	if err != nil {
		return FunctionProfile{},
			fmt.Errorf(
				"failed to build median feature vector: %w",
				err,
			)
	}
...
func BuildFunctionProfileFromSamples(
	...
	for index, sample := range samples {
		...
		vector, err := BuildProfilingFeatureVector(
			sample,
		)
	...
	return AggregateProfilingFeatureVectors(
		vectors,
	)
}

func validateVectorIdentity(
	vector ProfilingFeatureVector,
) error {
	if vector.SchemaVersion != ProfilingFeatureVectorSchemaVersion {
		return fmt.Errorf(
			"unsupported feature vector schema version %d",
	...
	if !finitePositive(
		vector.FunctionConfiguration.ConfiguredCPUs,
	) {
		return fmt.Errorf(
	...
	if vector.FunctionConfiguration.ConfiguredMemoryMB <= 0 {
		return fmt.Errorf(
			...
			vector.FunctionConfiguration.ConfiguredMemoryMB,
		)
	}
...
func sameFunctionConfiguration(
	first InvocationFunctionConfiguration,
	second InvocationFunctionConfiguration,
) bool {
	const cpuTolerance = 1e-9

	return math.Abs(
		first.ConfiguredCPUs-
			second.ConfiguredCPUs,
	) <= cpuTolerance &&
		first.ConfiguredMemoryMB ==
			second.ConfiguredMemoryMB
}
...
func randomForestFeaturesFromValues(
	values []float64,
) (RandomForestProfilingFeatures, error) {
	...
	if len(values) != expectedFeatureCount {
		return RandomForestProfilingFeatures{},
			fmt.Errorf(
				"expected %d feature values, got %d",
				expectedFeatureCount,
				len(values),
			)
	}
