...
func ExportInvocationSample(
	...
	if path == "" {
		return fmt.Errorf(
			"profiling export path cannot be empty",
		)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"failed to marshal profiling sample: %w",
			err,
		)
	}
	...
	if directory != "." {
		if err :=
			...
			return fmt.Errorf(
				"failed to create profiling export directory %s: %w",
				directory,
				err,
			)
		}
	...
	if err != nil {
		return fmt.Errorf(
			"failed to open profiling export file %s: %w",
			path,
			err,
		)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"failed to append profiling sample to %s: %w",
			path,
			err,
		)
	}
	...
	if written != len(line) {
		return fmt.Errorf(
			"short profiling sample write to %s: wrote %d of %d bytes",
			path,
			written,
			len(line),
		)
