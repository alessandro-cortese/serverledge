...
// schema version.
func LoadInvocationSamplesJSONL(
	path string,
) ([]InvocationSample, error) {
	path =
		strings.TrimSpace(
			path,
		)
	...
	if path == "" {
		return nil,
			fmt.Errorf(
				"profiling sample input path cannot be empty",
			)
	}
	...
	if err != nil {
		return nil,
			fmt.Errorf(
				"failed to open profiling sample dataset %s: %w",
				path,
				err,
			)
	}
	...
	// A profiling sample can contain many nested metrics. Increase the Scanner
	// limit so a valid JSONL record is not rejected by the default 64 KiB cap.
	scanner.Buffer(
		make(
			[]byte,
			64*1024,
	...
	for scanner.Scan() {
		...
		if err :=
			...
			return nil,
				fmt.Errorf(
					"invalid profiling sample JSON at line %d: %w",
					lineNumber,
					err,
				)
		}
		...
		if sample.SchemaVersion !=
			...
			return nil,
				fmt.Errorf(
					"unsupported invocation sample schema version %d at line %d: expected %d",
	...
	if err :=
		...
		return nil,
			fmt.Errorf(
				"failed to read profiling sample dataset %s: %w",
				path,
				err,
			)
	}
...
func LoadInvocationSamplesJSONLFiles(
	paths []string,
) ([]InvocationSample, error) {
	if len(paths) == 0 {
		return nil,
			fmt.Errorf(
				"at least one profiling sample input dataset is required",
			)
	}

	normalizedPaths := make(
		[]string,
		0,
		len(paths),
	)
	...
	for _, path := range paths {
		...
		if err != nil {
			return nil,
				fmt.Errorf(
					"failed to resolve profiling sample input path %s: %w",
					path,
					err,
				)
		}
		...
		if _, exists := seenPaths[cleanPath]; exists {
			return nil,
				fmt.Errorf(
					"duplicate profiling sample input path %s",
					cleanPath,
				)
		}
	...
	for _, path := range normalizedPaths {
		samples, err := LoadInvocationSamplesJSONL(
			path,
		)
		...
		for index, sample := range samples {
			...
			if requestID == "" {
				return nil,
					fmt.Errorf(
						"profiling sample at index %d in %s has empty request ID",
						index,
						path,
					)
			}
			...
			if previousPath, exists :=
				requestSources[requestID]; exists {

				return nil,
					fmt.Errorf(
						"duplicate request ID %q across profiling datasets %s and %s",
						requestID,
						previousPath,
						path,
					)
...
func DiscoverInvocationSampleDatasets(
	root string,
) ([]string, error) {
	...
	if root == "" {
		return nil,
			fmt.Errorf(
				"profiling dataset root cannot be empty",
			)
	}
	...
	if err != nil {
		return nil,
			fmt.Errorf(
				"failed to inspect profiling dataset root %s: %w",
				root,
				err,
			)
	}
	...
	if !info.IsDir() {
		return nil,
			fmt.Errorf(
				"profiling dataset root is not a directory: %s",
				root,
			)
	}

	targetName := filepath.Base(
	...
	if err != nil {
		return nil,
			fmt.Errorf(
				"failed to discover profiling datasets under %s: %w",
				root,
				err,
			)
	}
...
func BuildFunctionProfilesByGroup(
	samples []InvocationSample,
	samplesPerProfile int,
) (FunctionProfileBuildResult, error) {
	if samplesPerProfile <
	...
	for index, sample := range samples {
		...
		key :=
			functionProfileGroupKey{
				...
				ConfiguredCPUs: sample.
					FunctionConfiguration.
					ConfiguredCPUs,

				ConfiguredMemoryMB: sample.
					FunctionConfiguration.
					ConfiguredMemoryMB,
			}
	...
	for _, key := range keys {
		...
		if err != nil {
			return FunctionProfileBuildResult{},
				fmt.Errorf(
					"failed to aggregate function %q on machine tag %q: %w",
					key.FunctionName,
					key.MachineTag,
					err,
				)
...
// duplicate profiles.
func ExportFunctionProfilesJSONL(
	path string,
	profiles []FunctionProfile,
) error {
	path =
	...
	if path == "" {
		return fmt.Errorf(
			"function profile output path cannot be empty",
		)
	}
	...
	if directory != "." {
		if err :=
			...
			return fmt.Errorf(
				"failed to create function profile export directory %s: %w",
				directory,
				err,
			)
		}
	...
	tempFile, err :=
		os.CreateTemp(
			directory,
			".function-profiles-*.tmp",
		)

	if err != nil {
		return fmt.Errorf(
			"failed to create temporary function profile dataset: %w",
			err,
		)
	}
	...
	for index, profile := range profiles {
		...
		if profile.SchemaVersion !=
			...
			return fmt.Errorf(
				"function profile at index %d has unsupported schema version %d",
				index,
		...
		if err :=
			...
			return fmt.Errorf(
				"failed to encode function profile at index %d: %w",
				index,
				err,
			)
		}
	...
	if err :=
		...
		return fmt.Errorf(
			"failed to sync temporary function profile dataset: %w",
			err,
		)
	}
	...
	if err :=
		...
		return fmt.Errorf(
			"failed to set function profile dataset permissions: %w",
			err,
		)
	}
	...
	if err :=
		...
		return fmt.Errorf(
			"failed to close temporary function profile dataset: %w",
			err,
		)
	}
	...
	if err :=
		...
		return fmt.Errorf(
			"failed to replace function profile dataset %s: %w",
			path,
			err,
		)
	}
