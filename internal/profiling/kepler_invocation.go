...
func NewInvalidKeplerInvocationEnergyProfile(
	containerID string,
	reason string,
	...
	return &KeplerInvocationEnergyProfile{
		...
		InvalidReason: strings.TrimSpace(
			reason,
		),

		ContainerID: normalizeKeplerContainerID(
			containerID,
		),
	}
}
...
func (c *KeplerClient) WaitForContainerEnergyDelta(
	ctx context.Context,
	before KeplerContainerSnapshot,
	...
	*KeplerInvocationEnergyProfile,
	...
	if pollInterval <= 0 {
		return nil,
			fmt.Errorf(
				"Kepler poll interval must be positive",
			)
	}
	...
	if containerID == "" {
		return nil,
			fmt.Errorf(
				"Kepler before snapshot has an empty container ID",
			)
	}

	if len(
		before.CPUJoulesByZone,
	) == 0 {

		return nil,
			fmt.Errorf(
				"Kepler before snapshot for container %q has no energy zones",
				containerID,
			)
	}
	...
	for zone, value := range before.CPUJoulesByZone {
		...
		if strings.TrimSpace(
			...
			return nil,
				fmt.Errorf(
					"Kepler before snapshot for container %q contains an empty energy zone",
					containerID,
				)
		}
		...
		if !finiteKeplerCounter(
			...
			return nil,
				fmt.Errorf(
					"Kepler before snapshot for container %q contains invalid counter %v for zone %q",
					containerID,
					value,
					zone,
				)
	...
	for {
		if err :=
			...
			if lastReadErr != nil {
				return nil,
					fmt.Errorf(
						"timed out waiting for Kepler refresh for container %q after %d attempts: last read error: %v: %w",
						containerID,
						attempts,
						lastReadErr,
						err,
			...
			return nil,
				fmt.Errorf(
					"timed out waiting for Kepler refresh for container %q after %d attempts: %w",
					containerID,
					attempts,
					err,
				)
...
func keplerEnergyDelta(
	before KeplerContainerSnapshot,
	after KeplerContainerSnapshot,
) (
	map[string]float64,
	bool,
	error,
	...
	if beforeContainerID == "" ||
		...
		return nil,
			false,
			fmt.Errorf(
				"Kepler energy snapshots must contain container IDs",
			)
	}
	...
	if beforeContainerID !=
		...
		return nil,
			false,
			fmt.Errorf(
				"Kepler container changed during energy measurement: before=%q after=%q",
				beforeContainerID,
				afterContainerID,
			)
	}
	...
	if len(
		...
		len(
			after.CPUJoulesByZone,
		) {

		return nil,
			false,
			fmt.Errorf(
				"Kepler energy-zone set changed for container %q",
				beforeContainerID,
			)
	}
	...
	for zone, beforeValue := range before.CPUJoulesByZone {
		...
		afterValue,
			ok :=
			after.CPUJoulesByZone[zone]

		if !ok {
			return nil,
				false,
				fmt.Errorf(
					"Kepler energy zone %q disappeared for container %q",
					zone,
					beforeContainerID,
				)
		}
		...
		if !finiteKeplerCounter(
			...
			return nil,
				false,
				fmt.Errorf(
					"Kepler energy zone %q contains an invalid counter",
					zone,
				)
		}
		...
		if afterValue <
			...
			return nil,
				false,
				fmt.Errorf(
					"Kepler energy counter regressed for container %q zone %q: before=%f after=%f",
					beforeContainerID,
					zone,
					beforeValue,
					afterValue,
